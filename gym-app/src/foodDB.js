...
export const FOOD_DB = [
  ...
  { cat:"유제품·보충제", key:"프로틴쉐이크(WPI)", aliases:["프로틴","단백질쉐이크","웨이프로틴","쉐이크","프로틴쉐이크","프로틴 쉐이크"], unit:"count", baseQty:1, protein:24, carbs:3, sugar:1, fat:1, kcal:120, liquidMl:250 },
...
export function lookupLocalFoods(text, customEntries = []) {
  ...
  for (const seg of segments) {
    ...
    matched.push({
      ...
      liquidMl: entry.liquidMl ? Math.round(entry.liquidMl * mult) : 0,
    });
  }
...
export function makeCustomEntry({ name, cat, protein, carbs, sugar, fat, kcal, liquidMl }) {
  const e = { cat: cat || "기타", key: name, aliases: [name], unit: "count", baseQty: 1, protein, carbs, sugar, fat, kcal, custom: true };
  if (liquidMl && liquidMl > 0) e.liquidMl = liquidMl;
  return e;
}
